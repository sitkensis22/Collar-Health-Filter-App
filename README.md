...
## Description
Provides 3 classes of filtering for 9 different types of collar health alerts (mortality, resurrection, cluster, nsd, voltage, gps_accuracy, gps_transmission, gps_resurrection, and tag_release) in the data that were triggered by Collar Health Alert App: (1) filter-specific, (2) individual-specific, and (3) variable-specific.
...
### Settings 
**Set mortality-specific filter (`mortality`):** This logical input acts as a switch to turn on mortality alert filtering based on identifier(s) provided in the `mortality_id` input. 
...
**Set mortality resurrection-specific filter (`resurrection`):** This logical input acts as a switch to turn on mortality resurrection alert filtering based on identifier(s) provided in the `resurrection_id` input. 

**Identifier(s) for mortality resurrection-specifc filtering (`resurrection_id`):** This character string input is the track id(s) or serial number(s) (i.e., local_tag_identifier) to filter mortality resurrection alerts by. Note that multiple id(s) may be provided but they must be either the track id(s) or serial number(s) and not a mix of both. Multiple values must be comma-separated and input is only used if mortality resurrection filter is activated.

**Set cluster-specific filter (`cluster`):** This logical input acts as a switch to turn on cluster alert filtering based on identifier(s) provided in the `cluster_id` input. 
...
**Set net-squared displacement-specific filter (`nsd`):** This logical input acts as a switch to turn on cluster alert filtering based on identifier(s) provided in the `nsd_id` input. 
...
**Set voltage-specific filter (`voltage`):** This logical input acts as a switch to turn on voltage alert filtering based on identifier(s) provided in the `voltage_id` input. 
...
**Set GPS accuracy-specific filter (`gps_accuracy`):** This logical input acts as a switch to turn on GPS accuracy alert filtering based on identifier(s) provided in the `gps_accuracy_id` input. 
...
**Set GPS transmission-specific filter (`gps_transmission`):** This logical input acts as a switch to turn on GPS transmission alert filtering based on identifier(s) provided in the `gps_transmission_id` input. 
...
**Set GPS resurrection-specific filter (`gps_resurrection`):** This logical input acts as a switch to turn on GPS transmission alert filtering based on identifier(s) provided in the `gps_resurrection_id` input. 
...
**Set tag detachment date-specific filter (`tag_release`):** This logical input acts as a switch to turn on tag detachement date alert filtering based on identifier(s) provided in the `tag_release_id` input. 
...
**Set GPS individual-specific filter (`filter_specific`):** This logical input acts as a switch to turn on individual-specific alert filtering based on identifier(s) provided in the `filter_specific_id` input. 
...
**Setting `resurrection_id`:** If the identifier(s) are not present in the input dataset or not provided when the mortality resurrection filter switch is activated, an error will be returned. If the spelling does not match exactly, an error will be returned. Review available variables in the input dataset to confirm their existence and spelling. 
